...
function webviewLabelForTab(tabId: string) {
  return `browser-${tabId.replace(/[^a-zA-Z0-9-_]/g, "-")}`;
}

export function browserWebviewLabel(tabId: string) {
  return webviewLabelForTab(tabId);
}
...
export function BrowserWebviewHost({
  ...
  useEffect(() => {
    ...
    const ensureWebview = async () => {
      ...
      // Drop any previous instance for this label so URL changes remount cleanly.
      ...
      try {
        ...
        await new Promise<void>((resolve, reject) => {
          const timer = window.setTimeout(
            () => reject(new Error("create browser webview timeout")),
            10000,
          );
          ...
          webview!.once("tauri://error", (event) => {
            window.clearTimeout(timer);
            reject(event.payload ?? "create browser webview failed");
          });
        });
      } finally {
        creatingRef.current = false;
      }
      if (cancelled || !webview) {
      ...
      try {
        ...
        console.error("browser webview drag-drop listen failed", err);
      }
    ...
    void ensureWebview().catch((err) => {
      console.error("browser webview failed", err);
    });
  ...
  return (
    <div
      ref={hostRef}
      className={cn("absolute inset-0 h-full min-h-0 w-full bg-background", className)}
      aria-label="browser-webview-host"
    />
  );
}

export async function closeBrowserWebview(tabId: string) {
  const label = webviewLabelForTab(tabId);
